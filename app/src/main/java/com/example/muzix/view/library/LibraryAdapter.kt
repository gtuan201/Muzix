...
class LibraryAdapter(private val listItem : List<Any>) : RecyclerView.Adapter<ViewHolder>() {
    companion object{
    ...
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        return when(viewType){
            ...
            VIEW_TYPE_ARTIST -> {
                val binding = ItemArtistLibBinding.inflate(LayoutInflater.from(parent.context),parent,false)
    ...
    class PlaylistViewHolder(private val playlistBinding: ItemPlaylistLibBinding) : ViewHolder(playlistBinding.root){
        fun bind(playlist: Playlist){
            playlistBinding.tvNamePlaylist.text = playlist.name
            Glide.with(playlistBinding.imgPlaylist).load(playlist.thumbnail).placeholder(R.drawable.thumbnail).into(playlistBinding.imgPlaylist)
            playlistBinding.tvOwner.text = playlist.owner
        }
    }
    class ArtistViewHolder(private val artistBinding: ItemArtistLibBinding) : ViewHolder(artistBinding.root){
        fun bind(artist: Artist){
            artistBinding.tvNameArtist.text = artist.name
            Glide.with(artistBinding.imgArtist).load(artist.image).placeholder(R.drawable.thumbnail).into(artistBinding.imgArtist)
        }
    }
}